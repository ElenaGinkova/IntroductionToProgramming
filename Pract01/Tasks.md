...
int main()
{
  ...
  {
    std::cout << "Else"; // игнорират се наляво от запетаите
  }
}
```
...
3. Ползвайки тернарен оператор установете дали въведения символ от конзолата е малка латинска буква.
...
9. Напишете програма, която чете от конзолата температура в градуси Целзий и изведе каква е текущата климатична категория, базирана на следната скала:
 - Много горещо (т.е. >= 35°C)
 - Горещо (т.е. >= 25°C и < 35°C)
 - Топло (т.е. >= 15°C и < 25°C)
 - Хладно (т.е. >= 5°C и < 15°C)
 - Студено (т.е. < 5°C)
...
11. 
12. Да се напише програма, която чете от конзолата символ. Ако той е малка буква, извежда съответната голяма, а ако е голяма буква, извежда съответната малка, а ако не е буква, да изведе съобщение за невалиден символ.
13. Ще бъдат въведени секунди. Превърнете ги в следния формат часове:минути:секунди(hh:min:sec).
14. Въвеждат се час и минути. Добавете 5 минути към вече установения час. Принирайте новия час във формат hh:min.
15. h
